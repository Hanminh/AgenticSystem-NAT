"""native_optimize_deep_agent_v4 — GIỐNG v3, nhưng LLM DỰNG BẰNG LANGCHAIN + TẮT THINKING mạnh.

KHÁC v3 ở HAI điểm:
* **LLM dựng bằng LangChain** (`build_langchain_llm`) thay vì `llm_from_config` (get_llm của NAT):
  clone endpoint/model/api_key từ config rồi tạo MỘT `ChatOpenAI` mới (enable_thinking=False,
  temperature ghim). ChatOpenAI mới KHÔNG kèm metadata-hook/stream_usage của NAT.
* **Tắt thinking bằng `/no_think`** (soft-switch Qwen3, chèn đầu system prompt) — vì đã verify proxy
  LiteLLM NUỐT `chat_template_kwargs` (drop_params) nên chỉ enable_thinking=False KHÔNG đủ; `/no_think`
  tắt thinking bất kể proxy có forward chat_template_kwargs hay không. Tắt bằng `no_think=False`.

Giữ NGUYÊN mọi thứ khác của v3 (flat todos, resolve placeholder, lean profile, parallel_tool_calls
=False né bug drop '{', streaming adapter). Dùng để A/B với v3 khi thinking rò rỉ (`</think>`).
...
from agenticskills.langgraph_agents.optimize_agent_v2 import build_langchain_llm  # dựng ChatOpenAI bằng LangChain
...
NATIVE_OPTIMIZE_V4_SYSTEM_PROMPT = """\
...
# Builder
# --------------------------------------------------------------------------- #
def build_native_optimize_deep_agent_v4(
    ...
    *,
    ...
    if _n:
        logger.info("[native_optimize_v4] đã resolve '<skill_dir>' -> path tuyệt đối trong %d SKILL.md.", _n)

    builder = SyncBuilder.current()
    # --- LLM DỰNG BẰNG LANGCHAIN (khác v3 dùng llm_from_config) --------------------------------
    # Clone endpoint/model/api_key từ LLM trong NAT config, nhưng dựng MỘT `ChatOpenAI` MỚI bằng
    # `build_langchain_llm` với enable_thinking=False + temperature ghim. ChatOpenAI mới KHÔNG kèm
    # metadata-hook/stream_usage của NAT get_llm nên không cần gỡ.
    ...
    llm = build_langchain_llm(config_llm, enable_thinking=False, temperature=temperature,
                              label="native deep v4 LLM (LangChain)")

    # parallel_tool_calls=False: né bug parser vLLM DROP dấu '{' của tool call thứ 2 (song song)
    # -> args hỏng -> 'Extra data'. (Bật lại bằng NAT_DEEP_ALLOW_PARALLEL_TOOLS=1.)
    if os.getenv("NAT_DEEP_ALLOW_PARALLEL_TOOLS") not in ("1", "true", "True"):
        try:
            ...
            logger.warning("[native_optimize_v4] không đặt được parallel_tool_calls=False.")
    ...
    if enable_todos:
        if todos_flat_schema:
            ...
            logger.info("[native_optimize_v4] enable_todos=True (schema PHẲNG) -> `write_todos(todos_text)` "
            ...
            try:
                ...
                logger.warning(
                    "[native_optimize_v4] enable_todos=True + todos_flat_schema=False -> `write_todos` "
                ...
                logger.warning("[native_optimize_v4] không import được TodoListMiddleware -> bỏ qua todos.")
    ...
    logger.info(
        "[native_optimize_v4] skills=%s | llm=%s (từ config, thinking/temperature theo YAML) | "
    ...
    base_prompt = system_prompt or (NATIVE_OPTIMIZE_V4_SYSTEM_PROMPT if enable_todos
                                    else NATIVE_OPTIMIZE_SYSTEM_PROMPT)

    # TẮT THINKING (điểm khác biệt của v4): proxy LiteLLM hay NUỐT `chat_template_kwargs`
    # (drop_params) -> chỉ enable_thinking=False KHÔNG đủ. Chèn soft-switch `/no_think` của Qwen3
    # vào ĐẦU system prompt -> tắt thinking bất kể chat_template_kwargs có tới vLLM hay không.
    # (Điều kiện: bản Qwen tôn trọng /no_think — chuẩn họ Qwen3.)
    final_prompt = f"/no_think\n\n{base_prompt}" if no_think else base_prompt
...
def as_nat_graph_todos(graph: Any, *, stream_nodes=("model", ), strip_think: bool = True,
    ...
    Dùng thay `as_nat_graph` cho agent v2::
        ...
        my_agent = as_nat_graph_todos(build_native_optimize_deep_agent_v4(skills_subdir="Telecom_Skills"))
    """
